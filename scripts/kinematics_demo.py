...
class InverseKinematics:
    def __init__(self):
        ...
        self.goal_locations [[0.2, 0.2, 0.3], [0.2, -0.2, 0.3], [0.2, 0.2, 0.1], [0.2, -0.2, 0.1], [0, 0, 0.3]] 
    ...
    def run(self):
        ...
        while len(self.goal_location) != 0:
            goal_location = self.goal_locations[0]
            # run gradient descent after finding the right xyz to pick up the object
            self.do_gradient_descent(self.current_joint_angles, goal_location) # also takes into account picking up object
        
            # after gradient descent has converged, move the angles to the location for pickup
            self.move_group_arm.go(self.current_joint_angles, wait=True)
            rospy.sleep(5)

            self.goal_locations.pop()
        print('finished executing all movements')
